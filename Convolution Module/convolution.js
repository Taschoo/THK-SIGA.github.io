...
function setup() {
  ...
  width = container.offsetWidth * 0.98;
  height = width / 4;
  ...
  slidert0 = document.getElementById("t0");
  sliderT = document.getElementById("T");
  ...
  document.getElementById("htDropdown").value = "rect";
  document.getElementById("htDropdown").value = "rect";
}
...
function draw() {
  ...
  text('a1', map(a1, xMin, xMax, 0, width), 0.8 * height);
  text('b1', map(b1, xMin, xMax, 0, width), 0.85 * height);
  text('a2', map(a2, xMin, xMax, 0, width), 0.85 * height);
  text('b2', map(b2, xMin, xMax, 0, width), 0.8 * height);

}
let isCos = false; // used in function convolustion
function setSt() {
  document.getElementById("t0").value = (-4).toString();
  let selectedOption = document.getElementById("stDropdown").value;
...
function setHt() { 
  document.getElementById("T").value = (1).toString();
  let selectedOption = document.getElementById("htDropdown").value;
  if (selectedOption === "rect") {
    isDirac = false;
    ht = rectImpuls;
  } else if (selectedOption === "triangular") {
