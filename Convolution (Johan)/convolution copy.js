...
let optionText = {
  "rect": "Rechteck Impuls",
  "triangular": "Dreieck Impuls",
  "parabola": "Parabel Funktion",
  "dirac": "Dirac Impuls",
  "cosinus": "Cosinus Funktion"
};

let accentColor = getComputedStyle(document.documentElement).getPropertyValue('--AccentColor').trim();
...
function setHt(selectedOption) { 
  ...
  document.querySelector('#ht').innerText = "h(t): " + optionText[selectedOption];
}


let isCos = false; // used in function convolustion
function setSt(selectedOption) { 
  slidert0.value(-3); // restarts the convolution
  ...
  document.querySelector('#st').innerText = "s(t): " + optionText[selectedOption];
}
